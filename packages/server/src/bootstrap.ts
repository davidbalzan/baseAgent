...
// packages/server/src → repo root (3 levels up)
...
/**
 * The object returned by bootstrapAgent().
 * All agent infrastructure is wired up and ready — only serve() and signal
 * handlers remain to be called by the app layer.
 */
export interface AgentBootstrap {
  ...
  /** Pre-wired Hono app: /health, /dashboard, /run, /resume, /api/*, /webhook */
  app: Hono;
  /** Closes MCP handles, stops channel adapters and heartbeat. */
  shutdown: () => Promise<void>;
}

/**
 * Bootstrap all agent infrastructure and return a ready-to-use AgentBootstrap.
 *
 * This factory does NOT call serve() or register signal handlers — the
 * calling app layer is responsible for both.
 *
 * @param configPath Optional path to the YAML config file.
 *                   Defaults to `<repo-root>/config/default.yaml`.
 */
export async function bootstrapAgent(configPath?: string): Promise<AgentBootstrap> {
  ...
  registry.register(createAddMcpServerTool({ registry, mcpHandles, configPath: resolvedConfigPath }));
  ...
  // 11. Graceful shutdown — returned to the caller (app layer registers signal handlers)
